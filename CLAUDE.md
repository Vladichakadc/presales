...
- The Aruba page is modelled directly on the Fortinet one (same tab layout, same throughput-ladder widget) but sizes a different thing: it separates what the appliance **processes** from what the WAN links **carry**, because Path Conditioning's FEC parity adds to the second and Boost subtracts from it while lowering the first. `server/seed/legacyData/aruba.js` documents both effects; the page computes them together and derives the subscription bandwidth tier from the resulting WAN figure.
