...
Presales is a set of internal network-equipment presales tools — sizing calculators, a BOM/quote generator, and a design guide — for Huawei, Cisco, Fortinet, MikroTik, Nokia, Juniper, and Arista gear. It's an Express + SQLite backend serving a set of vanilla HTML/CSS/JS pages behind a shared-credential login, deployed as a single Railway service, mirroring the architecture of the sibling projects `chikisdtv` and `credifuturo`. UI text is in Spanish (`lang="es"`); code identifiers (functions, variables) are in English.
